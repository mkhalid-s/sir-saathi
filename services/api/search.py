...
def validate_search_scope(request: SearchRequest) -> None:
    ...
    if request.ac_number is None and request.part_number is None:
        raise ValueError("name search must be scoped by AC or polling-station part")
...
def search_records(request: SearchRequest, records: list[InternalVoterRecord]) -> list[PublicVoterRecord]:
    ...
    for record in records:
        if record.state_id != request.state_id:
            continue
        if request.ac_number is not None and record.ac_number != request.ac_number:
            continue
