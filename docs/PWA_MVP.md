...
- Every page carries the non-secret `PUBLIC_RELEASE_COMMIT` identity. A real-origin build requires a full lowercase Git SHA; the API exposes its independently configured SHA, and deployment probing rejects stale or mixed PWA/API releases.
