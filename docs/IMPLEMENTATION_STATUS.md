...
| Guidance and forms | Situation-based, date-aware guidance, complete governed state timelines, and the canonical enumeration/Form 6/Form 7/Form 8 reference with official portal actions are shared by the PWA and API. Missing schedule dates remain omitted rather than inferred. Guidance is advisory and always points back to official channels. |
...
The review snapshot passes 255 tests, reports zero npm vulnerabilities, verifies 140 governed source records as fresh, precaches 55 generated offline assets, and reports zero automated accessibility or discoverability findings across 40 indexable English pages plus the no-index error page.
