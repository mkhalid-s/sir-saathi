...
The schema-v3 template starts unapproved and requires a real HTTPS origin, the exact full deployed commit, the successful 90-check deployment-probe report bound to that commit, distinct operator and reviewer identities, explicit attestations that no voter data was used and public indexed search remained disabled, and evidence for all 12 build, database, edge, offline, backup/restore, monitoring, accessibility, feedback, and rollback checks. The feedback check requires a private reference proving an authorized owner and a synthetic privacy-safe acknowledgement/remediation/retest exercise; the validator cannot infer or create that evidence. A failed check needs a remediation reference and passing retest. Validator output contains only stable blocker IDs and counts; it does not echo the origin, people, infrastructure details, or private evidence notes. This record proves rehearsal completeness, not public-search authorization or accessibility conformance.
...
CI also starts empty disposable PostgreSQL 16 and Redis 8 services. `scripts/check_postgres_integration.py` first proves all eight migrations are pending, applies them through the production migration runner, proves the follow-up plan is clean, verifies `pg_trgm` and the required tables, and exercises the independently reviewed enable constraint and append-only authorization-event trigger with synthetic jurisdiction metadata. It asserts that the voter table remains empty. `scripts/check_redis_integration.py` executes the production Lua limiter, verifies atomic counts, expiry/retry behavior, isolated buckets, and non-consuming readiness, then removes every synthetic test key. Both emit only stable redacted results. These are real dependency-execution checks; they do not replace production TLS/authentication, migration dry run, pre-migration backup, restore drill, limiter-loss rehearsal, or operator review.
